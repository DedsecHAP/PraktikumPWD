...
function getApiData($format) {
	if ($format == "JSON") {
		$url = "http://localhost/PWD/Responsi/Aplikasi-Pendaftaran-Zakat-Berbasis-Website/api/getDataJSON.php";
		$request = curl_init($url);
		curl_setopt($request, CURLOPT_RETURNTRANSFER, 1);
		$response = curl_exec($request);
		$result = json_decode($response, true);
	} else {
		$url = "http://localhost/PWD/Responsi/Aplikasi-Pendaftaran-Zakat-Berbasis-Website/api/getDataXML.php";
